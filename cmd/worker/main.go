...
func newRootCmd() *cobra.Command {
	...
	rootCmd.PersistentFlags().IntVarP(&workerCount, "workers", "w", 1, "Number of workers to start")
	rootCmd.AddCommand(newUserCmd(workerCount))
	rootCmd.AddCommand(newGroupCmd(workerCount))

	return rootCmd
}

// newUserCmd creates a new user worker command.
func newUserCmd(workerCount int) *cobra.Command {
	cmd := &cobra.Command{
		Use:   UserWorker,
	...
	cmd.AddCommand(
		&cobra.Command{
			...
			Run: func(_ *cobra.Command, _ []string) {
				runWorkers(UserWorker, UserWorkerTypeFriend, workerCount)
			},
		},
		&cobra.Command{
			...
			Run: func(_ *cobra.Command, _ []string) {
				runWorkers(UserWorker, UserWorkerTypeGroup, workerCount)
			},
		},
	)
...
func newGroupCmd(_ int) *cobra.Command {
