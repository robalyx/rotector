...
import (
	...
	"github.com/robalyx/rotector/internal/setup/client/interceptor/roverse/scripts"
...
func New(
	...
	if cfg.Roverse.Domain != "" {
		if cfg.Roverse.SecretKey == "" {
			panic(interceptorutil.ErrMissingSecretKey)
		}
...
func (m *Middleware) selectProxy(ctx context.Context) (*url.URL, int64, error) {
	if len(m.proxies) == 0 {
		return nil, -1, interceptorutil.ErrNoProxiesAvailable
	}
