...
//
// This file is derived from github.com/diamondburned/ningen/v3/states/member
// Licensed under the ISC License.
//
// Modified to resolve runtime panics, fix member list fetching bugs, improve
// code clarity, and follow Go best practices for context propagation.
...
var (
	...
	// ErrIndexOutOfRange is returned when a member list operation has an invalid index.
...
func NewState(state *state.State, h AddHandler) *State {
	s := &State{
		state:      state,
...
func (m *State) Subscribe(ctx context.Context, guildID discord.GuildID) {
...
// string.
func (m *State) SearchMember(ctx context.Context, guildID discord.GuildID, query string) {
	if query == "" {
		return
	}
	...
	go func() {
		...
		err := m.state.SendGateway(ctx, search)
		if err != nil {
...
func (m *State) RequestMember(ctx context.Context, guildID discord.GuildID, memberID discord.UserID) {
	...
	go func() {
		...
		err := m.state.SendGateway(ctx, &gateway.RequestGuildMembersCommand{
			GuildIDs:  []discord.GuildID{guildID},
			UserIDs:   memberIDs,
			Presences: m.RequestPresences,
		})
...
// there is none.
func (m *State) GetMemberListChunk(channelID discord.ChannelID) int {
	m.minFetchMu.Lock()
...
func (m *State) RequestMemberList(
	ctx context.Context, guildID discord.GuildID, channelID discord.ChannelID, chunk int,
) [][2]int {
	...
	if err == nil {
		...
		// Round the total up with ceiling division.
		total = (total + 99) / 100
	}
	...
	// Cap chunk if we have a total, but always request at least the first chunk [0, 99].
	// According to Discord's lazy guild protocol, the [0,99] range is always requested.
...
func (m *State) onListUpdate(ev *gateway.GuildMemberListUpdateEvent) {
	...
	for i, op := range ev.Ops {
		...
		case "SYNC":
			...
			for idx := range len(op.Items) {
				ml.items[start+idx] = op.Items[idx]
			}
		...
		case "INVALIDATE":
			...
			if start >= len(ml.items) {
				continue
			}

			end = min(end, len(ml.items))

			// Copy the old items into the Items field for future uses in other handlers.
			op.Items = append([]gateway.GuildMemberListOpItem{}, ml.items[start:end]...)
			ev.Ops[i] = op

			// Nullify the to-be-invalidated chunks.
			for idx := start; idx < end; idx++ {
				ml.items[idx] = gateway.GuildMemberListOpItem{}
			}
		...
		case "SYNC", "INSERT", "UPDATE":
			...
			for i, item := range items {
				if item.Member != nil {
					...
					if err := m.state.PresenceSet(ev.GuildID, &items[i].Member.Presence, update); err != nil {
						m.OnError(errors.Wrap(err, "failed to set presence"))
					}
				}
			}
		}
...
// onMembers is called a bit after RequestGuildMembers if the UserIDs field is filled.
...
func growItems(items *[]gateway.GuildMemberListOpItem, maxLen int) {
	...
	cpy = append(cpy, make([]gateway.GuildMemberListOpItem, delta)...)
	*items = cpy
}
