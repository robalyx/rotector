...
func (l *List) TotalVisible() int {
	...
	for _, group := range l.groups {
		if group.ID == "offline" {
			return int(l.memberCount) //nolint:gosec // Discord member counts will never overflow int
		}
	}
	// Else, we should only show the onlines.
	return int(l.onlineCount) //nolint:gosec // Discord member counts will never overflow int
}

// MemberCount returns the total number of members.
func (l *List) MemberCount() uint64 {
	l.mu.Lock()
	defer l.mu.Unlock()
...
func (l *List) OnlineCount() uint64 {
	l.mu.Lock()
	defer l.mu.Unlock()
...
// CountNil returns the number of nil items.
